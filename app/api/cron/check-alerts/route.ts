  ...
  try {
    ...
    for (const alert of alerts) {
      try {
        ...
        const priceResponse = await fetch(priceUrl, { next: { revalidate: 0 } })
        ...
        if (priceToCheck <= alert.target_price) {
          ...
          try {
            ...
            errors.push(`Failed to send email for alert ${alert.id}`)
          }
        }
        ...
        console.error(`[Cron] Error checking alert ${alert.id}:`, error)
        errors.push(`Error checking alert ${alert.id}`)
      }
    }
